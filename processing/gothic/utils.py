...
def goes_nc_to_zarr(in_dir, channels, startday, endday, month, year, 
    ...
    for day in range(startday,endday+1):
        ...
        for channel in channels:
            ...
            if not nc_files:
                print(f'No .nc files found for {channel} in {nc_dir}. Skipping...')
                continue

            existing_nc_files = [f for f in nc_files if os.path.exists(f)]
            missing_count = len(nc_files) - len(existing_nc_files)
            if missing_count:
                print(f'WARNING: {missing_count} files disappeared before open for {channel} in {nc_dir}.')

            if not existing_nc_files:
                print(f'No readable files left for {channel} in {nc_dir}. Skipping...')
                continue

            # Stream writes file-by-file to avoid keeping an entire day in memory.
            out_path = in_dir + f'{goes_model}/{channel}/' + out_name
            ...
            for f in existing_nc_files:
                try:
                    ...
                    print(f'WARNING: file missing at open time, skipping: {f}')
                except Exception as e:
                    print(f'WARNING: failed to process {f}: {e}')

            if not wrote_any:
                print(f'No datasets could be opened for {channel} in {nc_dir}. Skipping...')
                continue
...
def goes_rad_to_rgb(path, date, goes, location):
    ...
    ds_C02 = xr.open_zarr(path+C02_file)
    C05_file = f'C05/{goes}_C05_{location}_' + date + '.zarr'
    ds_C05 = xr.open_zarr(path+C05_file)
    ...
    # Already aligned, just fill NaNs
    ds_C13_resampled_filled = ds_C13_aligned.ffill(dim='y').ffill(dim='x')
    ds_C05_resampled_filled = ds_C05_aligned.ffill(dim='y').ffill(dim='x')

    # Create combined dataset by merging
    combined_ds = xr.merge([
        ds_C13_resampled_filled['tb'].rename('tb_C13'),
        ds_C02['ref'].rename('ref_C02'),
        ds_C05_resampled_filled['ref'].rename('ref_C05')
    ])
