"""
Functions to orthorectify GOES-R ABI images using a 

Downloading so I can debug some issues with the functions
...
def _get_scratch_work_dir():
    """Return a writable scratch directory for temporary and cache files."""
    env_dir = os.environ.get("ORTHO_SCRATCH_DIR")
    candidates = [
        env_dir,
    ...
    raise OSError("No writable scratch directory found. Set ORTHO_SCRATCH_DIR.")
...
def _nearest_index_1d(sorted_axis, values):
    """Return nearest indices on a sorted 1D axis without large broadcast arrays."""
    idx = np.searchsorted(sorted_axis, values)
    idx = np.clip(idx, 1, len(sorted_axis) - 1)
    left = sorted_axis[idx - 1]
    right = sorted_axis[idx]
...
def make_ortho_map(goes_filepath, dem_filepath, out_filepath=None):
    ...
    # FIX: GeoTIFFs have descending y-coordinates, flip to ascending for proper transformation
    if dem.y.values[0] > dem.y.values[-1]:
        logging.info("\nFlipping DEM y-axis from descending to ascending...")
    ...
    logging.info(f"Transform grid: {coarse_nx}x{coarse_ny} (vs ABI {len(abi_image.x)}x{len(abi_image.y)})")
    logging.info("...done")

    # Create 2D arrays of longitude and latitude from the DEM
    logging.info("\nCreate 2D arrays of longitude and latitude from the DEM")
    X, Y = np.meshgrid(dem_downsampled.x.values, dem_downsampled.y.values)
    Z = dem_downsampled.values  # elevation of each DEM grid cell
    logging.info("...done")
    ...
    # Use vectorized transformation (note: downsampled DEM means fewer pixels)
    vect_transform = np.vectorize(lambda lon, lat, z: LonLat2ABIangle(lon, lat, z, H, req, rpol, e, lon_0))
    abi_grid_x, abi_grid_y = vect_transform(X, Y, Z)
    
    logging.info("...done")
    logging.info(f"Transformed x range: {abi_grid_x.min()} to {abi_grid_x.max()}")
    logging.info(f"Transformed y range: {abi_grid_y.min()} to {abi_grid_y.max()}")
    
    # Use downsampled coordinates for output
    downsampled_x = dem_downsampled.x
    downsampled_y = dem_downsampled.y
    logging.info("...done")
...
def orthorectify_abi(goes_filepath, pixel_map, data_vars, out_filename=None):
    ...
    for var in data_vars:
        ...
        # Flatten the DEM coordinates
        dem_x_flat = pixel_map.dem_px_angle_x.values.ravel()
        dem_y_flat = pixel_map.dem_px_angle_y.values.ravel()
        
        # Get ABI coordinate arrays
        abi_x = abi_image.x.values
        abi_y = abi_image.y.values

        # Keep only points that are within the ABI scan-angle domain; otherwise set NaN.
        # This avoids snapping out-of-domain pixels to image edges (which looks like noise).
        x_pad = 0.5 * np.nanmedian(np.abs(np.diff(abi_x)))
        y_pad = 0.5 * np.nanmedian(np.abs(np.diff(abi_y)))
        valid_mask = (
            (dem_x_flat >= np.nanmin(abi_x) - x_pad)
            & (dem_x_flat <= np.nanmax(abi_x) + x_pad)
            & (dem_y_flat >= np.nanmin(abi_y) - y_pad)
            & (dem_y_flat <= np.nanmax(abi_y) + y_pad)
        )

        abi_var_flat = np.full(dem_x_flat.shape, np.nan, dtype=float)
        if np.any(valid_mask):
            valid_x = dem_x_flat[valid_mask]
            valid_y = dem_y_flat[valid_mask]
            x_idx = _nearest_index_1d(abi_x, valid_x)
            y_idx = _nearest_index_1d(abi_y, valid_y)
            abi_var_flat[valid_mask] = abi_image[var].values[y_idx, x_idx]
        
        # Reshape back to DEM grid shape
        abi_var_values = abi_var_flat.reshape(pixel_map.dem_px_angle_x.shape)
        logging.info(
            f"Mapped {np.count_nonzero(valid_mask)}/{valid_mask.size} pixels for {var}"
        )
        logging.info("...done")
        ...
        # If we are looking at an ABI-L1b-Rad product, create either a reflectance (bands 1-6) or brightness temperature (bands 7-16) dataset
        if var == "Rad":
            # Bands 1-6 are reflective; negative radiance is non-physical for our workflow.
            if abi_image.band_id.values[0] <= 6:
                pixel_map[var] = pixel_map[var].clip(min=0)
    ...
    # Use manual nearest-neighbor indexing for consistency
    dem_x_flat = pixel_map.dem_px_angle_x.values.ravel()
    dem_y_flat = pixel_map.dem_px_angle_y.values.ravel()
    ...
    abi_fixed_grid_x_values = np.full(dem_x_flat.shape, np.nan, dtype=float)
    abi_fixed_grid_y_values = np.full(dem_y_flat.shape, np.nan, dtype=float)
    if np.any(valid_mask):
        valid_x = dem_x_flat[valid_mask]
        valid_y = dem_y_flat[valid_mask]
        x_idx = _nearest_index_1d(abi_x, valid_x)
        y_idx = _nearest_index_1d(abi_y, valid_y)
        abi_fixed_grid_x_values[valid_mask] = abi_x[x_idx]
        abi_fixed_grid_y_values[valid_mask] = abi_y[y_idx]
...
def ortho(
    ...
    if dem_filepath is None:
        scratch_dir = _get_scratch_work_dir()
        dem_filepath = os.path.join(
            scratch_dir,
            "temp_{demtype}_DEM.tif".format(demtype=demtype),
        )
        get_dem(
            demtype=demtype,
            bounds=bounds,
    ...
    cache_dir = os.path.join(_get_scratch_work_dir(), '.ortho_cache')
    try:
...
def make_ortho_map_zarr(goes_filepath, dem_filepath, out_filepath=None):
    """
    For the entire DEM, determine the ABI scan angle coordinates for every DEM grid cell, taking into account the underlying terrain and satellite's viewing geometry. Create the mapping between GOES-R ABI pixels (zarr input file) and a DEM grid (geotiff input file)

    Parameters
    ------------
    goes_filepath : str
        filepath to GOES ABI zarr file
    dem_filepath : str
        filepath to digital elevation model (DEM), GeoTiff file
    out_filepath : str
        optional filepath and netcdf filename to save this map to, defaults to None
...
def orthorectify_abi_zarr(zarr_filepath, pixel_map, data_vars, out_filename=None):
    """
    Using the pixel mapping for a specific ABI viewing geometry over a particular location,
    orthorectify the ABI radiance values and return an xarray dataarray with those values.
    ...
    data_vars : list
        list of variable names from the GOES ABI NetCDF file we wish to extract
    out_filename : str
        optional filepath and zarr filename to save the orthorectified image to, defaults to None
    ...
    # Open the GOES ABI image
    print("\nOpening GOES ABI image...\t\t\tABI image value\tPixel map value")
    abi_image = xr.open_dataset(zarr_filepath)
    ...
    # add back in the time coordinates
    pixel_map = pixel_map.assign_coords({"time": abi_image.time.values})
    pixel_map = pixel_map.reset_coords(drop=True)

    # Output this result to a new NetCDF file
    print("\nOutput this result to a new zarr file")
    if out_filename is None:
        out_filename = abi_image.dataset_name + "_ortho.zarr"
    print("Saving file as: {}".format(out_filename))

    pixel_map.to_zarr(out_filename)
    print("...done")
...
def ortho_zarr(
    ...
    """
    Wraps around get_dem(), make_ortho_map_zarr(), orthorectify_abi_zarr()
    ...
    goes_image_path : str
        filepath to GOES ABI zarr file
    data_vars : list
        list of variable names from the GOES ABI zarr file we wish to extract
    bounds : list
        longitude and latitude bounds to clip and orthorectify GOES ABI image, like [min_lon, min_lat, max_lon, max_lat]
    api_key : str
        Opentopography.org API key, can be created at https://portal.opentopography.org/requestService?service=api
    ...
    goes_ortho_map = make_ortho_map_zarr(goes_image_path, dem_filepath)

    # Apply the "ortho map" and save a new NetCDF file with data variables from the original file
    _ = orthorectify_abi_zarr(
        goes_image_path, goes_ortho_map, data_vars, out_filename=new_goes_filename
    )
