#!/usr/bin/env python
"""Submit one PBS job per month to download 2022 NLDAS-2/3 Gothic LW subsets."""
...
LOG_DIR = REPO / "analysis" / "logs_nldas_lw_download"
MONTH_RUNNER = REPO / "analysis" / "run_month_nldas_lw_download.sh"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--force", action="store_true", help="Overwrite existing cached LW files")
    parser.add_argument("--source", choices=("both", "nldas2", "nldas3"), default="both")
...
def submit_month(args: argparse.Namespace, month: int) -> str:
    ...
    job_name = f"nldaslw{args.year}{month:02d}"
    cmd = [
        "qsub",
        "-q",
        args.queue,
