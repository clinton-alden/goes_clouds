#!/usr/bin/env python
"""Download hourly NLDAS-2 forcing files for selected UTC hours.

This uses the same GES DISC OTF HTTP_services.cgi endpoint represented by the
existing files in /glade/u/home/cdalden/scratch/nldas, but saves normalized
filenames. Authentication is delegated to curl/Earthdata configuration on the
machine, typically ~/.netrc plus cookies.
"""
...
DEFAULT_OUT_DIR = Path("/glade/u/home/cdalden/scratch/nldas")
...
def output_name(dt: pd.Timestamp) -> str:
    return f"NLDAS_FORB0125_H.A{dt:%Y%m%d}.{dt:%H}00.020.nc.SUB.nc4"


def existing_match(out_dir: Path, dt: pd.Timestamp) -> Path | None:
    matches = sorted(out_dir.glob(f"*A{dt:%Y%m%d}.{dt:%H}00*"))
    return matches[0] if matches else None


def nldas2_url(dt: pd.Timestamp) -> str:
    year = dt.strftime("%Y")
    doy = f"{dt.dayofyear:03d}"
    base_file = f"NLDAS_FORB0125_H.A{dt:%Y%m%d}.{dt:%H}00.020.nc"
    filename = f"/data/NLDAS/NLDAS_FORB0125_H.2.0/{year}/{doy}/{base_file}"
    label = f"{base_file}.SUB.nc4"
    params = {
        "FILENAME": filename,
        "LABEL": label,
        "FORMAT": "bmM0Lw",
...
def main() -> None:
    ...
    print(f"Requested {len(times)} NLDAS2 hours from {times.min()} to {times.max()}", flush=True)
    for dt in times:
