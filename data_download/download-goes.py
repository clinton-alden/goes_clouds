...
for d in range(len(days)):
    for h in range(len(hours)):
        ...
        raw_files = [
            file for file in glob(filepath[i] + "*.nc")
            if not file.endswith("_ortho.nc")
        ]
        if not raw_files:
            ABI = ABI_Downloader(
            ...
            if os.path.exists(filepath[i]):
                ...
                # Orthorectified files may share this directory on retries but
                # no longer contain the original GOES projection variable
                # expected by subsetNetCDF. Never feed them back into clipping.
